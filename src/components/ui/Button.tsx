...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ variant = 'default', children, className = '', disabled, ...rest }, ref) => {
    ...
    const combinedClasses = `${baseClasses} ${variantClasses[variant]} ${disabled ? 'cursor-not-allowed' : ''} ${className}`

    return (
      <button ref={ref} className={combinedClasses} disabled={disabled} {...rest}>
        {children}
      </button>
    )
  }
